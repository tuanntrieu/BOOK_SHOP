...
public interface CartDetailRepository extends JpaRepository<CartDetail, Integer> {
    ...
    @Query("SELECT new com.example.projectbase.domain.dto.response.ProductFromCartResponseDto(cd.product.productId,cd.product.name,cd.product.featuredImage,cd.product.price,cd.product.discount,cd.quantity) FROM CartDetail cd WHERE cd.cart.id=?1 ORDER BY cd.createdDate DESC")
    ...
    @Query("SELECT new com.example.projectbase.domain.dto.response.ProductFromCartResponseDto(cd.product.productId,cd.product.name,cd.product.featuredImage,cd.product.price,cd.product.discount,cd.quantity) FROM CartDetail cd WHERE cd.cart.id=?1 AND cd.product.productId=?2")
